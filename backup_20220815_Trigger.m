...
classdef backup_20220815_Trigger < handle
    ...
    methods
        ...
        function t = Trigger(path_plx) 
            ...
            if ~isempty(regexp(plexonfile.AcquiringSoftware,'Zeus'))
                if  strcmp(plexonfile.EventChannels(1).Name, 'DIG01')
                    code_method = 1;
                else
                    code_method = 2;
                end
            ...
            switch code_method
                case 1
                    t.coreTriggerZeusDig;
                case 2
                    t.coreTriggerZeusAna;
                case 3
                    t.coreTriggerPlexAna;
            end   
...
%                     sort(unique(diff_raw))
        ...
        function coreTriggerZeusAna(t, path_plx)
            ...
            for m = 1:length(plexonfile.ContinuousChannels)
                ...
                if  strcmp(plexonfile.ContinuousChannels(m).Name, 'AIN02')
                    PULSE_LOCATION = m;
                    t.equipment = 'PLEXON';
                    t.raw = plexonfile.ContinuousChannels(PULSE_LOCATION);
                    t.info = t.extract; % not digital
