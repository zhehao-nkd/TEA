 classdef Experiment < handle
    ...
    properties
        ...
        config % 储存了各种configs， 比如 figure size blabla
...
%         signalGoodness
        ...
        inputs % inputs应作为保存输入的固定名称

        waves % 作为储存波形信息的固定名称

        timeSections % 如果Experiments是由多个recording拼接而成的，那么这个值代表了每个recording起始？的时间
        adfreq % the sampling frequency of SPKC channels
        nSections % 这个变量似乎是外部allocate的

        list % Expeirment版本的与Neuron的list类似的property
        info % 关于这个Experiment的信息
    ...
    methods % 内部计算方法
        ...
        function exp = Experiment(spikes, pl2data, folder_wav) % pl2data可以是path或Trigger
            
            %写Class构造函数的准则，分成几个模块，构造函数里只储存大方向，具体依靠每个模块实现
            dbstop if error
            ...
            % 保存输入值
            exp.inputs.spikes = spikes;
            exp.inputs.pl2data = pl2data;
            exp.inputs.folder_wav = folder_wav;


            exp.genEobjects; % % 获取与Songs有关的信息，并判断每个song folder所采用的trigger编码方式是二进制还是十进制
%             exp.getInfo;
%             exp.list = exp.toList;

            
           
            exp.knowWaves;      % 获取与waveform有关的信息    
            exp.getSoundlist;
...
%             exp.info = exp.getInfo;
        ...
        function exp = knowWaves(exp)
            ...
            exp.waves.waveform = exp.sp.waveform;
            exp.waves.pc = exp.sp.pc;
            exp.waves.peak = exp.sp.peak;
            exp.waves.valley = exp.sp.valley;
            exp.waves.times = exp.sp.time; % timestamps

        end
        
        function exp = genEobjects(exp)
            % 生成Piece（Ephys）objects，是核心代码 % 总之先写着，边写边改

            % 获取 Trigger Spike 这两种信息
            if strcmp(class(exp.inputs.pl2data) ,'string')||strcmp(class(exp.inputs.pl2data) ,'char') %如果喂的是path
                exp.tg = Trigger(exp.inputs.pl2data);
                [~,exp.info.pl2name,~] = fileparts(exp.inputs.pl2data);
            elseif strcmp(class(exp.inputs.pl2data) ,'Trigger') %如果喂的是处理后的Trigger Signal
                exp.tg = exp.inputs.pl2data;
                [~,exp.info.pl2name,~] = fileparts(t.inputpath);
            end

            exp.sp = Spike(exp.inputs.spikes); % here the neuron is a separatedT generated by spike.split
            exp.sp.set_ephys_fs(exp.tg.trigger_fs); % set sampling rate of ephys signal


             % 获取与Songs有关的信息，并判断每个song folder所采用的trigger编码方式是二进制还是十进制
            if strcmp(class(exp.inputs.folder_wav),'string')
            elseif strcmp(class(exp.inputs.folder_wav),'char')
                exp.inputs.folder_wav = convertCharsToStrings(exp.inputs.folder_wav);
            end

            e_collector = {};
            for fd = 1: length(exp.inputs.folder_wav)
                songpath = Sound.Extract(exp.inputs.folder_wav(fd));
                difftype = [];
                ...
                for k = 1: length(songpath)   %%% should be parfor!!!!!!!!!!
                    ...
                    sub_e{k} = Piece(exp.sp,exp.tg,sound);
                    %                     if isempty(sub_e{k})
                    %                         judger(k) = 0;
                    %                     else
                    %                         judger(k) = 1;
        ...
        function exp = getInfo(exp)

            % 这个方法收集了关于这个experiment的所有有用的信息

%             exp.info.numofcons =  length(find(~cellfun(@isempty, regexp(cellstr({exp.list.stimuliname}.'),'norm-CON'))));
%             exp.info.numofdegs = find((~cellfun(@isempty, regexp(cellstr({exp.list.stimuliname}.'),'deg'))));
%             exp.info.numoffrags = find((~cellfun(@isempty, regexp(cellstr({exp.list.stimuliname}.'),'frag'))));

            temp1 = unique(exp.inputs.spikes.channelname);
            exp.info.channelname = temp1{1};
            exp.info.unitname= unique(exp.sp.unit);  %??? 可能有错误
            exp.info.channelunit = strjoin({exp.info.channelname,num2str(exp.info.unitname)},'_');
            exp.info.neuronname = sprintf('%s_%s_%u',exp.info.pl2name,exp.info.channelname,exp.info.unitname);


            temp = regexp(exp.info.pl2name,'[RBOYRG]\d{3}','match');
            if ~isempty(temp)
                exp.info.birdid = temp{1};
            else
                exp.info.birdid = exp.info.pl2name;
            end
            ...
            if ~isempty(temp)&& length(exp.nSections) == 1 % 只有当前experiment不具有nSections时，才具有Fid这个性质
                exp.info.Fid = temp{1};
            elseif length(exp.nSections) > 1
                exp.info.Fid = nan;
            end



            % based on the Experiment.list, extract the useful information
            % about this experiment
 
        end
        
        function exp = shiftNeuron(exp,spikes, path_pl2, folder_wav,shift_value)
            % 纠正Zeus仪器的错误
            dbstop if error
            ...
            t = Trigger.shiftTrigger(path_pl2,shift_value);
            ...
            [~,exp.pl2name,~] = fileparts(path_pl2);
            exp.neuronname = sprintf('%s_%s_%u',exp.pl2name,exp.channelname,exp.unitname);
            ...
            exp.getSoundlist; % get a list of all songs
            ...
            temp = regexp(exp.pl2name,'[RBOYRG]\d{3}','match');
            if ~isempty(temp)
                exp.info.birdid = temp{1};
            else
                exp.info.birdid = exp.pl2name;
            end
        ...
        function exp = set_uniqueid(exp,unique_id) % I forgot the purpose of this function
            exp.info.uniqueid = unique_id;
            exp.info.unique_neuronname = sprintf('%s_%u',exp.info.birdid,exp.info.uniqueid);
        end
        
        function exp = getSoundlist(exp) % get sound list
        ...
        function exp = setFeatures_SAP(exp,sorted_data)% allocate sap features for each stimuli
            ...
            for k = 1: length(exp.e)
                ...
                if isempty(fileid)
                    continue
                elseif length(fileid) > 1 % 当同一个stimuli在不同F filenumber 里播放了多次
                    fileid = fileid(1);
                end
...
%                 
        ...
        function exp = setFeatures_SAT(exp) % MATLAB version
               % allocate sap features for each stimuli
...
%                 end
                % Generate raw features
                temp_feature = SAT_sound(exp.e{k}.rawy,exp.e{k}.fs).features;% 初始化，不修改
                exp.e{k}.rawfeatures.amplitude = temp_feature.amplitude;
                exp.e{k}.rawfeatures.pitch = temp_feature.pitch;
                exp.e{k}.rawfeatures.mean_frequency = temp_feature.mean_frequency;
                exp.e{k}.rawfeatures.FM = temp_feature.FM;
                exp.e{k}.rawfeatures.AM = temp_feature.AM;
                exp.e{k}.rawfeatures.goodness = temp_feature.goodness;
                exp.e{k}.rawfeatures.entropy = temp_feature.entropy;
                exp.e{k}.rawfeatures.peak_frequency = temp_feature.peak1; % ?????真的吗
                exp.e{k}.rawfeatures.aperiodicity = temp_feature.aperiodicity; % ?????真的吗
...
%                 
                % Generate features
                temp_feature = SAT_sound(exp.e{k}.y,exp.e{k}.fs).features; % 初始化， 然后修改
                exp.e{k}.features.amplitude = temp_feature.amplitude;
                exp.e{k}.features.pitch = temp_feature.pitch;
                exp.e{k}.features.mean_frequency = temp_feature.mean_frequency;
                exp.e{k}.features.FM = temp_feature.FM;
                exp.e{k}.features.AM = temp_feature.AM;
                exp.e{k}.features.goodness = temp_feature.goodness;
                exp.e{k}.features.entropy = temp_feature.entropy;
                exp.e{k}.features.peak_frequency = temp_feature.peak1; % ?????真的吗
                exp.e{k}.features.aperiodicity = temp_feature.aperiodicity; % ?????真的吗
%                 for ming = 1: length(fieldnames(exp.e{k}.features))
%                     fnames = fieldnames(exp.e{k}.features);
%                     values = exp.e{k}.features.( fnames{ming});
%                     if isnumeric( values) && length(values)> 1
...
%                 
    ...
    methods(Hidden)% deprecated methods
        ...
        function Deprecated_drawInSongFragsMeanFeaturesVsRespAsLineChart(exp) % draw the distribution of mean features
            ...
            saveas(gcf,sprintf('V2-InSongLineChartMeanFeaturesVsResp-%s.png',exp.info.unique_neuronname));
            close(gcf);
        end
    ...
    methods % 有外部输出的计算方法
        ...
        function thelist = toList(exp,whether_update_figure) % This is the API between class Neuron and class Experiment
            ...
            thelist = struct;
            eobjects = exp.e;
            channelname = exp.info.channelname;
            Fid = exp.info.Fid;
            timeSections = sort(exp.timeSections);
            for k = 1: length(eobjects)
                
                thelist(k).stimuliname = eobjects{k}.sound.name;
                
                thelist(k).fs = eobjects{k}.fs;
                thelist(k).judgelaten = eobjects{k}.judgelaten;
                
                thelist(k).pl2name = eobjects{k}.trigger.pl2name;
                thelist(k).channelname = channelname;
                thelist(k).unitname = eobjects{k}.spike.unit;
                ...
                thelist(k).pltext = eobjects{k}.pltext;
                
                thelist(k).features = eobjects{k}.features;
                thelist(k).rawfeatures = eobjects{k}.rawfeatures;
                thelist(k).pltfeatures = eobjects{k}.pltfeatures;
                thelist(k).meanfeatures = eobjects{k}.meanfeatures;

                if ~isnan(Fid)
                    thelist(k).Fid = Fid;
                else
                     % 根据stimuli播放的时间与每个section的起始时间确定stimuli的位置
                    thelist(k).Fid = ['F',num2str(length(find(mean(eobjects{k}.trigger_onset) > timeSections)))];
                end
                thelist(k).label = nan;  % 初始化
                
%                 if whether_update_figure == 1
%                     eobjects{k}.pltthree(0);
%                     frame = getframe(gcf);
%                     close(gcf);
%                     thelist(k).image = frame.cdata;
%                     thelist(k).imagename = eobjects{k}.sound.name;
%                 end
            ...
            for k = 1: length(exp.e)
               ...
               % nameparts = strcat(namepart1,"-",namepart2);
                ...
                for kk = 1: length(frags)
                    ...
                    frags(kk).fragnum = kk;
                    frags(kk).fs = E.fs;
                    frags(kk).pl2name = E.trigger.pl2name;
        ...
        function fix_segmentation(exp) % to fix the segmentation of songs into elements
            ...
            %temp = regexp(exp.pl2name,'[BRYOG]\d{3}','match');
            % birdid = temp{1};
        ...
        function meanWL = calMeanWaveLength(exp) % Calculate mean wavelength
            ...
            unpack = @(x) x{1};
            temp = unpack(regexp(exp.pl2name,'[ZP]\d{2}F\d{1}','match'));
            ZorP = unpack(regexp(temp,'[ZP]','match'));
            if strcmp(ZorP,"Z")
                exp.info.pl2_data_fs = 30000; %hard code !!!!!! Dangerous
            elseif strcmp(ZorP,"P")
                exp.info.pl2_data_fs = 40000;
            end
            meanWL =  mean(wavlen_units*(1/exp.info.pl2_data_fs)*1000); % ms
        ...
        function figmat = writeFigdata(exp)
            ...
            figure('Position',exp.config.figsize,'Color','w');
        ...
        function [others,singlestruct] = collectImages(exp)
            ...
            for r = 1: length(exp.e)
                ...
                singlestruct(r).pl2name = exp.pl2name;
    ...
    methods  % 作图方法
        ...
        function drawWaveform(exp)
            ...
            this_waveform =  exp.waves.waveform;
...
%             close(gcf);
            ...
            %clear I
            IMG = cell2mat(reshapedI);
            imwrite(IMG,sprintf('RawThree_%s.png',exp.info.neuronname));
    ...
    methods(Static) %静态方法  
        function sorted_data = extractFeaturesFromSapRawData(path_data_json_or_txt, path_info_json_or_txt)
            ...
            elseif strcmp(dataext, ".txt")
                data = readtable(path_data_json_or_txt); % 这里不要把table转换为struct, it is very slow!
            end
            ...
            The_file_index = [data.file_index].'; % 把这个值写到for 循环外面可以节省运行时间，因为 access the field of a struct is time consuming
            
            for k = 1: length(sorted_data)

                local_data = data( The_file_index == sorted_data(k).file_index,:); % allocate feature of this (k) stimuli
