 classdef Experiment < handle
    ...
    methods % 内部计算方法
        ...
        function exp = Experiment(spikes, pl2data, folder_wav) % pl2data可以是path或Trigger
            ...
            if strcmp(class(pl2data) ,'string')||strcmp(class(pl2data) ,'char') %如果喂的是path
                t = Trigger(pl2data);
                [~,exp.pl2name,~] = fileparts(pl2data);
            elseif strcmp(class(pl2data) ,'Trigger') %如果喂的是处理后的Trigger Signal
                t = pl2data;
                [~,exp.pl2name,~] = fileparts(t.inputpath);
            end
            ...
            if ~isempty(temp)
                ...
                exp.birdid = exp.pl2name;
            end
            
            exp.figsize = PM.size3plots;
            temp = regexp(exp.pl2name,'(?<=[ZP]\d+)F\d+','match');
            if ~isempty(temp)
                exp.Fid = temp{1};
            end
    ...
    methods % 有外部输出的计算方法
        ...
        function thelist = toList(exp,whether_update_figure) % This is the API between class Neuron and class Experiment
            ...
            for k = 1: length(exp.e)
                ...
                thelist(k).judgelaten = exp.e{k}.judgelaten;
                
                thelist(k).pl2name = exp.e{k}.trigger.pl2name;
...
%                 end
        ...
        function meanWL = calMeanWaveLength(exp) % Calculate mean wavelength
            ...
            temp = regexp(exp.pl2name,'[ZP]\d{2}F\d{1}','match');
            ZorP = regexp(temp,'[ZP]','match');
            if strcmp(ZorP,"Z")
                exp.pl2_data_fs = 30000; %hard code !!!!!! Dangerous
            elseif strcmp(ZorP,"P")
                exp.pl2_data_fs = 40000;
            end
            meanWL =  mean(wavlen_units*(1/exp.pl2_data_fs)*1000); % ms
