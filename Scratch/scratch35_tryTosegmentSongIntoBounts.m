% scratch to split songs into bouts

result_dir = "E:\WavsCollection";

subdirs = Extract.folder(result_dir); 

function autosegmenter_restrict_bout(input_folder,num_of_bouts)
   ...
   y_to_analyze = end_terminal/1000 + 0.1; % 0.1 is the compensation
   
   temp = split(input_folder,'\');
   birdid = temp{length(temp)};
   [rawy,~,I,syledge,eleedge] = drawAutoseg(y_to_analyze ,fs,birdid);
...
function [rawy,fiy,I,syledge,eleedge] = drawAutoseg(fiy,fs,birdid)
...
drawsylfig(I,meanbI,stdbI,sumbI,Imax,maxloc,syllable_threshold); % function
close(gcf)
% FIND INTERSECTIONS    <@> One way
used_for_seg = Imax;
...
draw_seg_result(edges,I,Imax,syllable_threshold) % % % % draw the result of primary segmentation
close gcf
...
% % % % % % % % % %     %after flipping
    ...
    if length(highx)== 1
        ...
        drawelefig(eimg,eimg_mean,eimg_std,eimg_ysum,eimg_mixed,e_max,e_maxloc,highx,highy...
...
% @_@_@_@_@_@_@
drawfinalfig(I,edges,tp_sum,birdid); % plot the final result
