classdef AutoSeg
    ...
    methods
        ...
        function standard(a) % 此乃标准算法

            %这个有rescrit bout的算法，现在不再适用了
            ...
            for idx = 1: length(filenames)
                ...
                if ~isempty(regexp(filenames{idx},'WNS', 'once'))  % if the name contain the 'WNS'
                    ...
                    segdata.motedge = [min(segdata.syledge)/2,...
                        fenjiedian1,fenjiedian2,...
                        max(segdata.syledge)+ (length(y)-max(segdata.syledge))/2];               
                ...
                % 生成初始的segdata.motedge,用来分割出 introductory notes, motif1,motif2
                %通过比较syllables之间gaps的长度，取四个点：
                % 最前，分界点1，分界点2，最后
                initials = syledge(1:2:end);
                ends = syledge(2:2:end);

                remove1st_initials = initials(2:end);
                removelast_ends = ends(1:end-1);
                [~,new_order] = sort(remove1st_initials-removelast_ends,'descend');
                fenjiedian1 = (removelast_ends(new_order(1)) + remove1st_initials(new_order(1)))/2;
                try
                    fenjiedian2 = (removelast_ends(new_order(2)) + remove1st_initials(new_order(2)))/2;
                catch
                    fenjiedian2 = fenjiedian1 + (max(segdata.syledge) - fenjiedian1)/2;
                end

                segdata.motedge = [min(segdata.syledge)/2,...
                    fenjiedian1,fenjiedian2,...
                    max(segdata.syledge)+ (length(y)/fs-max(segdata.syledge))/2];
...
%             figure; imshow(bI)
            ...
            %syllable_threshold = 0.08;%2023.01.20 0.09;% 至2023.01.18之前是0.10;
            syllable_threshold = 0.075;
...
%                         ,lowx,lowy,tsm_highx,flip_highy,tsm_lowx,flip_lowy,tpoint_x,trans_flip_tpoint_x )
    ...
    methods %Deprecated
        ...
        function Deprecated_standard(a,num_of_bouts ) % 此乃标准算法
