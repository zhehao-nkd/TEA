 classdef Neuron < handle
    ...
    methods
        ...
        function n = Neuron(spikes, path_plx, folder_wav,mergeIdx)
            dbstop if error
        ...
        %             end
            ...
            if ~isempty(n.mergeIdx)
                t = Trigger(path_plx,n.mergeIdx);
            else
                t = Trigger(path_plx);
            end
           ...
           for k = 1: length(songs)   %%% should be parfor!!!!!!!!!!
                ...
                so = Sound(songs{k},global_coding_method);
                e{k} = Ephys(sp,t,so);
...
%             sylinf =  horzcat(collect{:});
        ...
        function [e_songs,slist_songs] = onlyExtractEphysOfSongs(n)
            ...
            song_only_ids =  find(~cellfun(@isempty, regexp({n.slist.name}.','norm')));
...
%             end
        ...
        function IMG = three(n) % draw three plot
            ...
            for idx = 1: length(n.e)
                n.e{idx}.three;%_with_sig_resp; % newer version of threeplot drawing method
                frame = getframe(gcf);
                I{idx} = frame.cdata;
                close(gcf);
            end
...
%                 
        ...
        function info = todisplay(n) % This is the API between class Analysis and class Neuron
...
%             
        ...
        function drawInSongFragsMeanFeaturesVsRespAsLineChart(n) % draw the distribution of mean features
...
%            plot(znum_toshow.');
    ...
    methods(Static) 
        ...
        function sorted_data = extractFeaturesFromSapRawDataJson(path_data_json, path_info_json)
            
            data = extract.json(path_data_json);
            info = extract.json(path_info_json);
            ...
            for k = 1: length(sorted_data)
                ...
                local_data = data(dataids);
                
                % allocate feature of this (k) stimuli
