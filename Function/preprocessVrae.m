...
function preprocessVrae(input)

if isstruct(input)
    datafilename = "syllablelist.mat";
    labeled_data.eleinf = input;

else

    loaded = load(input);
    labeled_data.eleinf = loaded.all_eleinf;
    datafilename = input;
end
...
save(dataSaveDir + "processed" + datafilename, "data", "labels", "frs", "spiketimes_std",'-v7.3')
