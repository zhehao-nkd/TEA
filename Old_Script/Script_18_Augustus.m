% What is senatus? Senatus is the selected representational songs 

Senatus_Dir_TwoMotif = "C:\Users\Zhehao\Dropbox (OIST)\My_Stimuli\R677@11232021\senatusTwoMotif";
TARGET_my_stimuli_folder_ONE = "C:\Users\Zhehao\Dropbox (OIST)\My_Stimuli\R677@11232021\senatusOneMotif";
% 下面的部分直接生成stimuli set
TARGET_DIR = "E:\R677_ELE";
mkdir(TARGET_DIR);ConDirs = fromWavFindParentFolder(Senatus_Dir_TwoMotif,"E:\Stimuli_Source\allBirdsSong");
...
% tic
% raw_ote_eleinf = categoFrags(noncon_ote_eleinf).eachsong;
% toc
% unique_ote_eleinf = stimuli.unique_it( raw_ote_eleinf );
...
% ote_eleinf = rmfield(rm_eleinf, 'len');
% %split ote_eleinf to non-con and con eleinf
% 
% 
% % 生成 con_spe_eleinf
% % 此外，这里的目的是为了让con-ELEINF的运算流程和ote-eleinf的运算流程一样
as = autoseg(TARGET_my_stimuli_folder_TWO);
as.standard;
raw_con_spe_eleinf = MetaStimuli.Eleinf(TARGET_my_stimuli_folder_TWO ,1,'SegData');
con_spe_eleinf = categoFrags(raw_con_spe_eleinf).eachsong;
parent_dir_con = subdirs(locb(locb>0));
con_ote_eleinf = MetaStimuli.Eleinf(parent_dir_con,1,'SegData');
...
% 
% as = autoseg(TARGET_my_stimuli_folder_ONE);
% as.standard;
% senatus_1_eleinf = MetaStimuli.Eleinf(TARGET_my_stimuli_folder_ONE ,1,'SegData');
% senatus_1_eleinf = categoFrags(senatus_1_eleinf).eachsong;
% 
% 
% 
% merge the Cons and Non COns together
all_eleinf = horzcat(con_spe_eleinf,ote_eleinf);
